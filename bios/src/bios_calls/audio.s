...
@ SWI 2Ah SoundGetJumpList - GBATEK "BIOS Sound Functions": writes 36
@ word-size pointers to sound driver functions to the destination
@ (0x120 bytes, 4-byte aligned). GBATEK does not document the entry
@ order; every entry resolves to the documented immediate-return
@ handler, matching this BIOS's driverless sound surface.
@ (Two upstream defects fixed here: entries were stored byte-indexed,
@ overlapping all 36 writes within 36 bytes; and the loop counter
@ clobbered r3, which the SWI dispatcher does not save.)
@ input: r0: destination address for function table
swi_SoundGetJumpList:
    ldr r2, =#swi_DoNothing
    mov r12, #35  @ 36 entries, but check with ge (for (int i = 35; i >= 0; i--))
    get_jump_list_loop:
        str r2, [r0, r12, lsl #2]
        subs r12, #1
...
@ SWI 1Bh SoundDriverMode - GBATEK "BIOS Sound Functions".
@ r0 = mode word: bits 0-6 reverb value + bit 7 apply-reverb flag,
@ bits 8-11 simultaneously-produced channels (1-12), bits 12-15
@ master volume (1-15), bits 16-19 playback-frequency index (1-12),
@ bits 20-23 D/A converter bits (8..11 = 9..6 output bits).
@ The driver bookkeeping fields established by SoundDriverInit are
@ updated (reverb at +6, max channels at +7, when a work area is
@ registered at [3007FF0h]); the D/A-bits field maps directly to its
@ hardware register, SOUNDBIAS bits 14-15 (value 8 = 9-bit/32.768kHz
@ ... 11 = 6-bit/262.144kHz). Master volume and frequency index have
@ no hardware register and no BIOS-resident mixer to consume them.
swi_SoundDriverMode:
    ldr r2, =#0x03007ff0
    ldr r2, [r2]
    cmp r2, #0
    ...
    @ reverb: bits 0-6, applied when bit 7 set
    tst r0, #0x80
    andne r1, r0, #0x7f
    strneb r1, [r2, #6]

    @ simultaneous channels: bits 8-11 (0 = leave unchanged)
    lsr r1, r0, #8
    ands r1, #0xf
    strneb r1, [r2, #7]

.sdm_hw:
    @ D/A converter bits: bits 20-23, values 8..11 -> SOUNDBIAS
    @ bits 14-15 resolution 0..3 (0 = leave unchanged)
    lsr r1, r0, #20
    ands r1, #0xf
    beq .sdm_done
    sub r1, #8
    ...
    bhi .sdm_done
    mov r12, #MMIO_BASE
    ldrh r2, [r12, #REG_SOUNDBIAS - MMIO_BASE]
    bic r2, #0xC000
    orr r2, r2, r1, lsl #14
    strh r2, [r12, #REG_SOUNDBIAS - MMIO_BASE]
.sdm_done:
    bx lr

@ SWI 1Ch SoundDriverMain - the BIOS-resident music driver's per-frame
@ mix entry ("call every 1/60 second" per GBATEK). This BIOS ships no
@ resident mixer: titles under test link their sound driver into the
@ cartridge and were instrumented never calling this entry (see
@ bios/README). Deterministic immediate return, registers preserved.
swi_SoundDriverMain:
    bx lr

@ SWI 1Eh SoundChannelClear - GBATEK: "Clears all direct sound
@ channels and stops the sound." Hardware-level: disable both sound
@ DMA channels and reset the two direct-sound FIFOs (SOUNDCNT_H bits
@ 11/15); when a SoundDriverInit-registered work area is present
@ (same ident guard as SoundDriverVSyncOff), its PCM buffer is
@ cleared as well.
swi_SoundChannelClear:
    ...
    mov r1, #MMIO_BASE
    mov r0, #0
    strh r0, [r1, #REG_DMA1CNT_H - MMIO_BASE]
    strh r0, [r1, #REG_DMA2CNT_H - MMIO_BASE]

    @ reset FIFO A + B
    ldrh r2, [r1, #REG_SOUNDCNT_H - MMIO_BASE]
    orr r2, #0x0800
    orr r2, #0x8000
    strh r2, [r1, #REG_SOUNDCNT_H - MMIO_BASE]

    @ ident-guarded PCM buffer clear (Smsh/Tmsh)
    ldr r4, =#0x03007ff0
    ldr r4, [r4]
    cmp r4, #0
    beq .scc_return
    ...
    mov r0, #0
    stmfd sp!, { r0 }       @ the zero lives at [sp]; it is CpuSet's fill source
    mov r0, sp
...
@ SWI 25h MultiBoot - GBATEK "BIOS Multi Boot": transfers and boots a
@ program image on consoles connected over the serial link (r0 =
@ MultiBootParam*, r1 = transfer mode; returns r0 = 0 on success,
@ 1 on failure). This BIOS targets hardware with no serial link
@ peers; the documented no-clients outcome is a failure return.
swi_MultiBoot:
    mov r0, #1
    bx lr

@ SWI 20h-24h - sound-related entries GBATEK lists as
@ "SoundWhatever0..4", explicitly undocumented. No caller observed
@ across the instrumented test corpus (see bios/README);
@ deterministic immediate return, registers preserved.
swi_SoundUndocumented:
    bx lr
...
swi_SoundDriverInit:
    ...
    mov r0, #0
    stmfd sp!, { r0 }       @ the zero lives at [sp]; it is CpuSet's fill source
    mov r0, sp              @ (upstream computed sp-4 here, one word below
                            @ the pushed zero - the fill read stack garbage)
...
@ SWI 19h SoundBias - GBATEK "BIOS Sound Functions"
@ r0 = 0: target bias level 0; r0 <> 0: target level 0x200
@ Ramp SOUNDBIAS bits 0-9 by +/-1 per step until the target is reached.
@ Upper bits (14-15 amplitude/sampling) are preserved.
@ Delay: GBATEK states GBA uses a fixed delay count of 8 (r1 is NDS/DSi
@ only). Implemented as a simple 8-iteration spin between steps.
swi_SoundBias:
    @ resolve target level into r0
    cmp r0, #0
    moveq r0, #0
    movne r0, #0x200

    mov r1, #MMIO_BASE
    ldrh r2, [r1, #REG_SOUNDBIAS - MMIO_BASE]
    ldr r12, =#0x3FF           @ level mask (bits 0-9)

.sound_bias_step:
    and r3, r2, r12            @ current level
    cmp r3, r0
    beq .sound_bias_done
    ...
    @ recombine with preserved non-level bits
    bic r2, r12
    orr r2, r3
    strh r2, [r1, #REG_SOUNDBIAS - MMIO_BASE]

    @ fixed delay count of 8 (GBATEK GBA convention)
    mov r3, #8
.sound_bias_delay:
    subs r3, #1
    bne .sound_bias_delay
